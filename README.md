# 视汇 (VFusion) - 通用跨隔离网数据交换与汇聚平台架构方案 (v0.20.0)
...
当前发布版本为 **v0.20.0**。本版本已完成 SHA-256 数据包校验升级、细粒度 Token Scope 与 RBAC 审计导出授权、CSV 注入防护、SQLite 数据库聚合统计与事务级联优化、.env 配置文件全面对齐以及免安装绿色发布包打包构建支持。
