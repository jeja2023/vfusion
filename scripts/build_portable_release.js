...
// Helper: 仅复制 Storage 下的 Schema 模板文件，排除生产数据库与数据文件
...
copyStorageTemplatesSync(path.join(ROOT_DIR, 'storage'), path.join(PORTABLE_DIR, 'storage'));
...
writeBatFileSync(path.join(PORTABLE_DIR, '一键无损升级(保留历史数据).bat'), batUpgrade);
...
writeBatFileSync(path.join(ROOT_DIR, '一键无损升级(保留历史数据).bat'), batUpgrade);
