...
### 3. HMAC 数字签名验伪校验
第三方可在 Header 中提取 `X-VFusion-Signature`，并用该 Webhook 节点独立的 HMAC 密钥验证消息来源防篡改。Webhook 密钥与跨网数据包 `hmac_secret` 相互独立，数据包密钥不能用于验证 Webhook。

新增 Webhook 节点或执行“轮换密钥”后，控制台只在本次响应中返回一次 64 位随机签名密钥；请通过安全配置渠道交给接收方，并按以下方式对原始 JSON 请求体计算 HMAC-SHA256：

```js
const signature = crypto.createHmac('sha256', webhookSecret).update(rawBody, 'utf8').digest('hex');
```

密钥轮换接口：`POST /api/webhooks/:id/rotate-secret`（仅管理员）。轮换完成后旧密钥立即失效。
