...
### 1. 认证与鉴权方式
内网中台支持两种鉴权方式：
* **Cookie Session 鉴权**：先调用 `/api/auth/login` 获取 Cookie，后续请求带上 Cookie。
* **Bearer Token 鉴权**：调用登录接口获取 Token 后，请求 Header 带上：
...
1. **接口返回 401 Unauthorized**：
   * 请检查 Header 是否包含了正确的 `Authorization: Bearer <token>` 或 Cookie。
2. **凭证照片 `files[i].url` 如何在第三方网页展示**：
   * `url` 为相对路径（如 `/assets/tasks/...`），在第三方系统中展示时拼接视汇中台 IP 及端口即可（如 `http://192.168.1.100:5002/assets/...`）。
3. **数据大屏定时拉取频率**：
