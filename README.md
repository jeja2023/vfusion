# 视汇 (VFusion) - 通用跨隔离网数据交换与汇聚平台架构方案 (v0.23.0)
...
> - 单据打包与解包：Zip 封装、SHA-256 校验和、HMAC-SHA256 签名与验签（兼容历史 MD5 包）
...
* [x] **Phase 1（底层中台引擎）**：实现通用解析引擎、解包摆渡、SHA-256+HMAC 校验与持久化（SQLite + JSON 文件）。
...
| 项目 | 当前状态 |
| :--- | :--- |
| 数据包防篡改 | HMAC-SHA256 签名写入 `info.json` 并强制验签，未签名/被篡改的包一律拒收并移入死信区；Webhook 使用每节点独立签名密钥 |
...
| 敏感信息 | FTP 口令不下发前端（以掩码返回）；`storage/` 已排除出版本控制；图片使用 10 分钟资产 Token |
